...
function check_hash() {
    local file="$1"
    local basemsg="Checksum for file %s %s!\n"

    printf "Checking checksum for file %s ...\n" "$file"

    if md5sum -c "$file" &>/dev/null; then
        printf "$basemsg" "$file" successful
    else
        printf "$basemsg" "$file" failed
        false
    fi
}
...
function file_present() {
    local file="$1"
    local basemsg="File %s %s!\n"

    if [ -f "$file" ]; then
        printf "$basemsg" "$file" present
    else
        printf "$basemsg" "$file" "not present"
        false
    fi
}
...
(
    ...
    hashurl="${baseurl}/${hashname}"
    hashxzurl="${baseurl}/${hashxzname}"
    ...
    if ! file_present "$hashname"; then
        download_file "$hashname" "$hashurl"
    fi

    if ! file_present "$hashxzname"; then
        download_file "$hashxzname" "$hashxzurl"
    fi

    if check_hash "$hashname"; then
        echo "Image already downloaded with valid checksum!"
        download=0
        extract=0
    fi

    if [ $extract = 1 ] && check_hash "$hashxzname"; then
        echo "Archive already downloaded with valid checksum!"
        download=0
    fi

    if [ $download = 1 ]; then
        download_file "$xzname" "$xzurl"

        # Check downloaded file
        check_hash "$hashxzname"
